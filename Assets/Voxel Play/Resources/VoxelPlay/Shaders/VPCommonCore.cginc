...
fixed GetSpecular(float3 normal, float3 viewDir) {
...
#if VOXELPLAY_PIXEL_LIGHTS && defined(IS_WATER) && !defined(IS_REALISTIC_WATER)
	half3 GetWaterSpecular(float3 normal, float3 worldPos) {
	...
	#define VOXELPLAY_APPLY_WATER_SPECULAR(color, norm, wpos) color.rgb += GetWaterSpecular(norm, wpos);
#else
	#define VOXELPLAY_APPLY_WATER_SPECULAR(color, norm, wpos)
#endif
