...
namespace VoxelPlay {
    ...
    public class UnityTerrainGeneratorEditor : Editor {
        ...
        public override void OnInspectorGUI() {
            ...
            if (!allIgnored) {
                if (needAssign) {
                    EditorGUILayout.HelpBox("'Assigned'로 지정된 리소스가 위 목록에서 올바르게 설정되었는지 확인하거나 'Refresh'를 눌러 TerrainData 정보를 다시 불러오세요.", MessageType.Warning);
                } else {
                    EditorGUILayout.BeginHorizontal();
                    ...
                    EditorGUILayout.EndHorizontal();
                    if (needCreate) {
                        EditorGUILayout.HelpBox("'Generate'를 눌러 지형 리소스용 복셀 정의를 생성하세요.", MessageType.Info);
        ...
        /// <summary>
        /// 투명한 색상이 텍스처 색상 배열의 첫 번째 색상이라고 가정합니다.
        /// </summary>
