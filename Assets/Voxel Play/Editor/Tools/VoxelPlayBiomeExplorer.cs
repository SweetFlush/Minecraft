    ...
    public class VoxelPlayBiomeExplorer : UnityEditor.EditorWindow {
        ...
        void OnGUI() {
            ...
            if (env == null) {
                ...
                EditorGUILayout.HelpBox("Biome Explorer가 현재 씬에서 Voxel Play Environment 인스턴스를 찾지 못했습니다.", MessageType.Error);
                GUIUtility.ExitGUI();
            } else {
                world = env.world;
            }
            ...
            if (world == null) {
                EditorGUILayout.HelpBox("Voxel Play Environment 인스턴스에 World Definition을 할당하세요.", MessageType.Warning);
                GUIUtility.ExitGUI();
            }
            ...
            EditorGUILayout.HelpBox("현재 설정을 기반으로 지형 생성과 바이옴 분포를 미리 봅니다.", MessageType.Info);
