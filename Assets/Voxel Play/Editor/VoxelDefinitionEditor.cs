    ...
    public class VoxelDefinitionEditor : Editor {
        ...
        public override void OnInspectorGUI () {
            ...
            if (vd.usesMicroVoxels) {
                Mesh mesh = vd.GetMicroVoxelsPreviewMesh();
                if (mesh != null) {
                    if (meshPreviewEditor == null) {
                    ...
                    if (gpuInstancing.boolValue) {
                        ...
                        if (createGameObject.boolValue && generateCollider.boolValue) {
                            ...
                            if (o != null && o.GetComponentInChildren<Collider>() != null) {
                                EditorGUILayout.HelpBox("프리팹에서 콜라이더가 발견되었습니다. 'Generate Collider' 옵션을 사용할 때는 제거하는 것을 고려하세요.", MessageType.Warning);
                            }
                        }
        ...
        void CheckTintColorFeature () {
            if (tintColor.colorValue.r < 1f || tintColor.colorValue.g < 1f || tintColor.colorValue.b < 1f) {
                ...
                if (e != null) {
                    if (!e.enableTinting) {
                        EditorGUILayout.HelpBox("Voxel Play Environment 컴포넌트에서 Tint Color 셰이더 기능이 비활성화되어 있습니다.", MessageType.Warning);
                    }
                }
            }
        }
        ...
        string GetRenderTypeDescription (RenderType rt) {
            switch (rt) {
                case RenderType.Opaque: return "빛을 통과시키지 않는 완전 불투명한 큐브 복셀입니다. 상단, 하단, 6개 면에 공통으로 사용하는 세 번째 텍스처 등 세 가지 텍스처를 지원합니다.";
                case RenderType.OpaqueAnimated: return "빛을 통과시키지 않는 완전 불투명한 큐브 복셀로, 세 가지 텍스처 애니메이션을 지원합니다.";
                case RenderType.Opaque6tex: return "빛을 통과시키지 않는 완전 불투명한 큐브 복셀입니다. 각 면마다 하나씩 총 6개의 텍스처를 지원합니다.";
                case RenderType.OpaqueNoAO: return "빛을 통과시키지 않는 완전 불투명한 큐브 복셀입니다. 앰비언트 오클루전이나 글로벌 일루미네이션을 지원하지 않아 어두워지지 않습니다.";
                case RenderType.Cloud: return "구름 렌더링에 특화된 렌더 타입입니다.";
                case RenderType.Transp6tex: return "투명 큐브 복셀입니다. 각 면마다 하나씩 총 6개의 텍스처를 지원하며, 텍스처의 알파 값이 투명도를 결정합니다.";
                case RenderType.Water: return "물 렌더링에 사용됩니다.";
                case RenderType.Cutout: return "컷아웃 큐브 복셀로, 주로 나뭇잎이나 구멍이 있는 복셀에 사용됩니다.";
                case RenderType.CutoutCross: return "식생 렌더링을 위해 예약된 타입입니다. 두 개의 쿼드를 사용해 덤불을 렌더링합니다.";
                case RenderType.Invisible: return "아무것도 렌더링하지 않지만 콜라이더를 생성할 수 있습니다.";
                case RenderType.Custom: return "오브젝트, 하프 블록, 스타일리시한 나무나 식생 등 커스텀 형태에 사용됩니다. 프리팹이 사용하는 머티리얼이 렌더링에 사용됩니다(유효한 머티리얼은 커스텀 복셀 관련 온라인 문서를 참조하세요).";
                default:
                    return "알 수 없는 렌더 타입입니다!";
            }
        }
        ...
        public override Texture2D RenderStaticPreview (string assetPath, Object[] subAssets, int width, int height) {
            ...
            EditorUtility.CopySerialized(vd.icon != null ? vd.icon : vd.textureSide, tex);
