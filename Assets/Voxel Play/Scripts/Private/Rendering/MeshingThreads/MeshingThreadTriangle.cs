...
namespace VoxelPlay {
    ...
    public class MeshingThreadTriangle : MeshingThread {
        ...
        /// <summary>
        /// 청크 메쉬를 생성합니다.필요한 경우 라이트맵도 계산합니다.
        /// </summary>
        public override void GenerateMeshData () {
            ...
            for (int y = 0; y < VoxelPlayEnvironment.CHUNK_SIZE; y++) {
                ...
                for (int z = 0; z < VoxelPlayEnvironment.CHUNK_SIZE; z++) {
                    ...
                    for (int x = 0; x < VoxelPlayEnvironment.CHUNK_SIZE; x++, voxelIndex++) {
                        ...
                        if (useMicroVoxels) {
                            if (microVoxels.needsMeshDataUpdate) {
                                microVoxels.needsMeshDataUpdate = false;
                                ulong mvHash = microVoxels.GetGridHashCode();
                                if (!env.microVoxelsPrototypes.TryGetValue(mvHash, out microVoxels.prototype)) {
                                    microVoxelMesher.UpdateMeshData(microVoxels);
                                    env.microVoxelsPrototypes[mvHash] = microVoxels.prototype;
                                }
                            }
                        ...
                        if (waterLevel > 0) {
                            ...
                            if (hb == 0) {
                                if (v1b == FULL_OPAQUE) {
                                    ...
                                    AddFaceWater(Cube.faceVerticesBack, Cube.normalsBack, pos, indicesWater, typeWater.textureIndexSide, light + noflow, 0, corner_height_bl, 0, corner_height_br);
#if USES_TINTING
                            ...
                            if (hf == 0) {
                                if (v1f == FULL_OPAQUE) {
                                    ...
                                    AddFaceWater(Cube.faceVerticesForward, Cube.normalsForward, pos, indicesWater, typeWater.textureIndexSide, light + noflow, 0, corner_height_fr, 0, corner_height_fl);
#if USES_TINTING
                                        tempChunkColors32.AddRange(faceColors);
#endif
                                }
                            ...
                            if (hl == 0) {
                                if (v1l == FULL_OPAQUE) {
                                    ...
                                    AddFaceWater(Cube.faceVerticesLeft, Cube.normalsLeft, pos, indicesWater, typeWater.textureIndexSide, light + noflow, 0, corner_height_fl, 0, corner_height_bl);
#if USES_TINTING
                                        tempChunkColors32.AddRange(faceColors);
#endif
                                }
                            ...
                            if (hr == 0) {
                                if (v1r == FULL_OPAQUE) {
                                    ...
                                    AddFaceWater(Cube.faceVerticesRight, Cube.normalsRight, pos, indicesWater, typeWater.textureIndexSide, light + noflow, 0, corner_height_br, 0, corner_height_fr);
#if USES_TINTING
                                        tempChunkColors32.AddRange(faceColors);
#endif
                                }
                            ...
                            if (chunk_top_middle_middle[top_middle_middle].typeIndex <= Voxel.HoleTypeIndex || (wh < 15 && th == 0)) {
                                ...
                                AddFaceWater(Cube.faceVerticesTop, Cube.normalsUp, pos, indicesWater, typeWater.textureIndexSide, light + foam + flow, corner_height_bl, corner_height_fl, corner_height_br, corner_height_fr);
                                AddFaceWater(Cube.faceVerticesTopFlipped, Cube.normalsUp, pos, indicesWater, typeWater.textureIndexSide, light + flow, corner_height_fl, corner_height_bl, corner_height_fr, corner_height_br);
#if USES_TINTING
                                        tempChunkColors32.AddRange(faceColors);
                                        tempChunkColors32.AddRange(faceColors);
#endif
                            ...
                            if (chunk_bottom_middle_middle[bottom_middle_middle].typeIndex <= Voxel.HoleTypeIndex) {
                                AddFaceWater(Cube.faceVerticesBottom, Cube.normalsDown, pos, indicesWater, typeWater.textureIndexSide, light + noflow, 0, 0, 0, 0);
#if USES_TINTING
                                        tempChunkColors32.AddRange(faceColors);
#endif
                            }
                            ...
                            case RenderType.Opaque6tex: {
                                    ...
                                    if (allowAO || doNotPackVertices) {
                                        ...
                                        if (v1b < FULL_OPAQUE || useMicroVoxels) {
                                            ...
                                            if (useMicroVoxels) {
                                                Vector3[] normals = Cube.rotatedNormals[(Cube.normalsBackIndex + rotationIndex) % 4];
                                                AddMicroVoxelGeometry(microVoxelsVertexData[(int)Cube.Side.Back].vertices, microVoxelsVertexData[(int)Cube.Side.Back].uvs, normals, pos, indices, resolvedTextureIndex + extraData, l0, l1, l2, l3, enableColliders, rotationIndex, tintColor);
                                            } else {
                                        ...
                                        if (v1f < FULL_OPAQUE || useMicroVoxels) {
                                            ...
                                            if (useMicroVoxels) {
                                                Vector3[] normals = Cube.rotatedNormals[(Cube.normalsForwardIndex + rotationIndex) % 4];
                                                AddMicroVoxelGeometry(microVoxelsVertexData[(int)Cube.Side.Forward].vertices, microVoxelsVertexData[(int)Cube.Side.Forward].uvs, normals, pos, indices, resolvedTextureIndex + extraData, l0, l1, l2, l3, enableColliders, rotationIndex, tintColor);
                                            } else {
                                                if (doNotPackVertices) {
                                                    AddFaceWithAO(Cube.faceVerticesForward, Cube.normalsForward, pos, indices, resolvedTextureIndex + extraData, l0, l1, l2, l3);
#if USES_TINTING
                                        ...
                                        if (v1l < FULL_OPAQUE || useMicroVoxels) {
                                            ...
                                            if (useMicroVoxels) {
                                                Vector3[] normals = Cube.rotatedNormals[(Cube.normalsLeftIndex + rotationIndex) % 4];
                                                AddMicroVoxelGeometry(microVoxelsVertexData[(int)Cube.Side.Left].vertices, microVoxelsVertexData[(int)Cube.Side.Left].uvs, normals, pos, indices, resolvedTextureIndex + extraData, l0, l1, l2, l3, enableColliders, rotationIndex, tintColor);
                                            } else {
                                                if (doNotPackVertices) {
                                                    AddFaceWithAO(Cube.faceVerticesLeft, Cube.normalsLeft, pos, indices, resolvedTextureIndex + extraData, l0, l1, l2, l3);
#if USES_TINTING
                                        ...
                                        if (v1r < FULL_OPAQUE || useMicroVoxels) {
                                            ...
                                            if (useMicroVoxels) {
                                                Vector3[] normals = Cube.rotatedNormals[(Cube.normalsRightIndex + rotationIndex) % 4];
                                                AddMicroVoxelGeometry(microVoxelsVertexData[(int)Cube.Side.Right].vertices, microVoxelsVertexData[(int)Cube.Side.Right].uvs, normals, pos, indices, resolvedTextureIndex + extraData, l0, l1, l2, l3, enableColliders, rotationIndex, tintColor);
                                            } else {
                                                if (doNotPackVertices) {
                                                    AddFaceWithAO(Cube.faceVerticesRight, Cube.normalsRight, pos, indices, resolvedTextureIndex + extraData, l0, l1, l2, l3);
#if USES_TINTING
                                        ...
                                        if (v1u < FULL_OPAQUE && type.supportsBevel) {
                                            ...
                                            if (useMicroVoxels) {
                                                AddMicroVoxelGeometry(microVoxelsVertexData[(int)Cube.Side.Top].vertices, microVoxelsVertexData[(int)Cube.Side.Top].uvs, Cube.normalsUp, pos, indices, resolvedTextureIndex + extraData, l0, l1, l2, l3, enableColliders, rotationIndex, tintColor);
                                            } else {
                                                if (doNotPackVertices) {
                                                    AddFaceWithAO(Cube.faceVerticesTop, Cube.normalsUp, pos, indices, resolvedTextureIndex + extraData, l0, l1, l2, l3);
#if USES_TINTING
                                        ...
                                        if (v1d < FULL_OPAQUE || useMicroVoxels) {
                                            ...
                                            if (useMicroVoxels) {
                                                AddMicroVoxelGeometry(microVoxelsVertexData[(int)Cube.Side.Bottom].vertices, microVoxelsVertexData[(int)Cube.Side.Bottom].uvs, Cube.normalsDown, pos, indices, resolvedTextureIndex + extraData, l0, l1, l2, l3, enableColliders, rotationIndex, tintColor);
                                            } else {
                                                if (doNotPackVertices) {
                                                    AddFaceWithAO(Cube.faceVerticesBottom, Cube.normalsDown, pos, indices, resolvedTextureIndex + extraData, l0, l1, l2, l3);
#if USES_TINTING
            ...
            for (int k = 0; k < VoxelPlayEnvironment.MAX_MATERIALS_PER_CHUNK; k++) {
                if (env.renderingMaterials[k].greedyMesherLitAO != null) {
                    env.renderingMaterials[k].greedyMesherLitAO.FlushTriangles(tempChunkVertices, meshJobs[jobIndex].indexBuffers[k], tempChunkUV0, tempChunkNormals, tempChunkColors32);
                }
                if (env.renderingMaterials[k].greedyMesherLit != null) {
                    env.renderingMaterials[k].greedyMesherLit.FlushTriangles(tempChunkVertices, meshJobs[jobIndex].indexBuffers[k], tempChunkUV0, tempChunkNormals, tempChunkColors32);
                }
        ...
        void AddMicroVoxelGeometry (List<Vector3> vertices, List<Vector2> uvs, Vector3[] normals, Vector3 pos, List<int> indices, int textureIndex, float w0, float w1, float w2, float w3, bool addColliderData, int rotationIndex, Color32 tintColor) {
            Vector4 uv;
            ...
            for (int k = 0; k < verticesCount; k += 4) {
