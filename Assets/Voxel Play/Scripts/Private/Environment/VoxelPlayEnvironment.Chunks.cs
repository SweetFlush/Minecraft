...
namespace VoxelPlay {
    ...
    public partial class VoxelPlayEnvironment : MonoBehaviour {
        ...
        /// <summary>
        /// 존재하는 경우 청크를 가져오거나 forceCreation이 true로 설정된 경우 청크를 생성합니다.
        /// </summary>
        /// <returns><c>진실</c>, if chunk fast was gotten, <c>거짓</c> otherwise.</returns>
        /// <param name="chunkX">청크 x.</param>
        /// <param name="chunkY">덩어리와.</param>
        /// <param name="chunkZ">청크 z.</param>
        /// <param name="createIfNotAvailable">If set to <c>진실</c> force creation if chunk doesn't exist.</param>
        ...
        /// </summary>
        /// <returns>덩어리.</returns>
        /// <param name="hash">해시시.</param>
        /// <param name="chunkX">청크 x.</param>
        /// <param name="chunkY">덩어리와.</param>
        /// <param name="chunkZ">청크 z.</param>
        /// <param name="createEmptyChunk">If set to <c>진실</c> create empty chunk.</param>
        /// <param name="complete">If set to <c>진실</c> detail generators will fire as well as OnChunkCreated event. Chunk will be marked as populated and a refresh will be triggered if within view distance.</param>
        VoxelChunk CreateChunk (int hash, int chunkX, int chunkY, int chunkZ, bool createEmptyChunk, bool complete = true) {
            ...
            if (createEmptyChunk) {
                ...
                if (world.infinite || (position.x >= -world.extents.x && position.x <= world.extents.x && position.y >= -world.extents.y && position.y <= world.extents.y && position.z >= -world.extents.z && position.z <= world.extents.z)) {
                    ...
                    if (!chunkHasContents) {
                        if (!chunk.isCloud && world.terrainGenerator != null) {
        ...
        public void ChunksExportAll () {
            ...
            settings.globalSpecularIntensity = Shader.GetGlobalFloat(ShaderParams.VPGlobalSpecular);
