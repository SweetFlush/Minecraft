...
namespace VoxelPlay {
    ...
    public partial class VoxelPlayEnvironment : MonoBehaviour {
        ...
        /// <summary>
        /// 모든 청크와 내부 엔진 구조를 파괴합니다. 종료 중에만 사용해야 합니다.
        /// </summary>
        ...
        /// <summary>
        /// forceChunkDistance 거리까지 플레이어 위치 주변의 청크를 확인합니다.
        /// </summary>
        ...
        /// <summary>
        /// 지형 생성기에서 직접 특정 위치에 대한 하이트맵 정보를 가져옵니다(캐시된 값을 사용하지 않음).
        /// </summary>
        /// <returns>높이 맵 정보입니다.</returns>
        ...
        /// <summary>
        /// 특정 위치에 대한 하이트맵 정보를 가져옵니다.이 방법은 런타임에만 작동합니다.Editor에서 GetTerainInfo()를 사용하세요.
        /// </summary>
        /// <returns>높이 맵 정보입니다.</returns>
        /// <param name="x">x 좌표입니다.</param>
        /// <param name="z">z 좌표입니다.</param>
        ...
        /// <summary>
        /// 청크의 16x16 위치에 대한 하이트맵 정보를 가져옵니다.
        /// </summary>
        /// <returns>높이 맵 정보입니다.</returns>
        /// <param name="x">청크의 가장 왼쪽 위치의 x 좌표입니다.</param>
        /// <param name="z">청크의 가장 뒤쪽 위치의 z 좌표입니다.</param>
        /// <param name="heights">HeightMapInfo 값의 배열입니다.</param>
        public void GetHeightMapInfoFast (double x, double z, out HeightMapInfo[] heights) {
            GetHeightMapInfoFast(x, z, out heights, out _);
        }
        ...
        /// <param name="z">청크의 가장 뒤쪽 위치의 z 좌표입니다.</param>
        /// <param name="heights">HeightMapInfo 값의 배열입니다.</param>
        /// <param name="heightsIndex">배열에 있는 첫 번째 HeightMapInfo 값의 인덱스입니다.</param>
        public void GetHeightMapInfoFast (double x, double z, out HeightMapInfo[] heights, out int heightsIndex) {

            if (heightMapCache.TryGetValue(x, z, out heights, out heightsIndex)) return;

            VoxelPlayTerrainGenerator tg = world.terrainGenerator;
            int index = 0;
            for (int zz = 0; zz < CHUNK_SIZE; zz++) {
                for (int xx = 0; xx < CHUNK_SIZE; xx++, index++) {
                    tg.GetHeightAndMoisture(x + xx, z + zz, out float altitude, out float moisture);
                    if (altitude > 1f)
                        altitude = 1f;
                    else if (altitude < 0f)
                        altitude = 0f;
        ...
        /// <summary>
        /// 주어진 x, z 위치에 대해 캐시된 높이 데이터를 제거합니다.
        /// </summary>
        /// <param name="x"></param>
        /// <param name="z"></param>
