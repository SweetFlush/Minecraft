    ...
    public partial class VoxelPlayFirstPersonController : VoxelPlayCharacterControllerBase {
        ...
        protected virtual void LateUpdateImpl() {
            ...
            if (env.cameraHasMoved || forceUpdateCrosshair > 0) {
                ...
                crosshairOnBlock = env.RayCast(ray, out _crosshairHitInfo, hitRange, colliderTypes: ColliderTypes.IgnorePlayer, layerMask: crosshairHitLayerMask, microVoxels: microVoxels > 0) && _crosshairHitInfo.voxelIndex >= 0;
                if (changeOnBlock) {
                    if (crosshairOnBlock) {
            ...
            if (crosshairOnBlock) {
                ...
                if (voxelHighlight) {
                    env.VoxelHighlight(ref _crosshairHitInfo, voxelHighlightColor, voxelHighlightEdge, microVoxelSize: microVoxels);
                }
            } else {
