...
namespace VoxelPlay {
    ...
    public class TextureArrayPacker {
        ...
        /// <summary>
        /// 단일 텍스처 배열에 허용되는 최대 텍스처
        /// </summary>
        public int maxTextures => SystemInfo.maxTextureArraySlices;

        /// <summary>
        /// 이 텍스처 패커가 더 많은 텍스처를 허용하지 않으면 true를 반환합니다.
        /// </summary>
        ...
        /// <summary>
        /// 텍스처 목록의 인덱스와 전체 인덱스(목록의 인덱스 + 법선/변위 맵의 존재를 지정하는 일부 플래그)를 반환합니다.
        /// </summary>
        ...
        Color32[] CombineNormalsAndElevation(Texture2D normalMap, Texture2D elevationMap) {
            ...
            } else if (normalMap.width != settings.textureSize || normalMap.height != settings.textureSize) {
            ...
            } else if (elevationMap.width != settings.textureSize || elevationMap.height != settings.textureSize) {
            ...
            if (QualitySettings.activeColorSpace == ColorSpace.Linear) {
                for (int k = 0; k < normalMapColorsLength; k++) {
                    normalMapColors[k].r = (byte)(255 * Mathf.LinearToGammaSpace(normalMapColors[k].r / 255f));
                    normalMapColors[k].g = (byte)(255 * Mathf.LinearToGammaSpace(normalMapColors[k].g / 255f));
                    normalMapColors[k].b = (byte)(255 * Mathf.LinearToGammaSpace(normalMapColors[k].b / 255f));
                    normalMapColors[k].a = (byte)(255 * Mathf.LinearToGammaSpace(normalMapColors[k].a / 255f));
                }
            }
