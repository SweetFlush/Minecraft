...
namespace VoxelPlay {
    ...
    public partial class VoxelPlayFirstPersonController : VoxelPlayCharacterControllerBase {
        ...
        [Header("Climbing")]
        [Tooltip("플레이어가 복셀을 뛰어넘지 않고도 언덕을 오를 수 있는 경우")]
        public bool canClimb = true;
        [Tooltip("등반에 허용되는 최대 계단 높이")]
        public float climbMaxStepHeight = 1.1f;
        [Tooltip("'빠른 홉'을 수행하는 대신 상승 전환을 원활하게 만듭니다.")]
        public bool smoothClimb = true;
        [Tooltip("원활한 등반을 시작하기 위한 최소 고도차")]
        public float climbYThreshold = 0.5f;
        [Tooltip("원활한 등반을 위한 속도")]
        ...
        /// <summary>
        /// 이 메소드는 캐릭터 컨트롤러가 게임 객체에 연결되어 있는지 확인하고 공개 "hasCharacterController" 필드를 업데이트합니다.
        /// </summary>
        /// <returns></returns>
        ...
        /// <summary>
        /// 플레이어 이동/콜라이더와의 상호작용을 허용하기 전에 플레이어 청크가 완료되었는지 확인합니다.
        /// </summary>
        ...
        protected virtual void UpdateWithCharacterController() {
            ...
            if (input.focused && input.enabled) {
                ...
                if (env.buildMode && input.GetButtonDown(InputButtonNames.MicroVoxels)) {
                    ToggleMicroVoxels();
                    if (microVoxels > 0) {
        ...
        protected virtual void DoHit(int damage) {
            ...
            env.RayHit(ray, damage, maxDistance, player.GetHitDamageRadius(), microVoxels: microVoxels, microVoxelDestroyProb: microVoxelsProb);
        }
        ...
        public override void ToggleMicroVoxels() {
            base.ToggleMicroVoxels();
            ResetCrosshairPosition();
        }

        public override void ToggleMicroVoxels(int size, float probability) {
            base.ToggleMicroVoxels(size, probability);
            ResetCrosshairPosition();
        }
